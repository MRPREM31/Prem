...
export const requestPushPermission = async () => {
  try {
    ...
    // Retry polling for Push Subscription ID if subscribed but not yet generated by OneSignal
    if (isSubscribed && !subscriptionId) {
      console.log("[OneSignal] Opted in but subscription ID is not yet ready. Polling...");
      for (let i = 0; i < 10; i++) {
        await new Promise(r => setTimeout(r, 500));
        subscriptionId = OneSignal.User?.PushSubscription?.id || null;
        if (subscriptionId) {
          console.log("[OneSignal] Successfully retrieved subscription ID via polling:", subscriptionId);
          break;
        }
      }
    }
